...
{
    while (isalnum(data[i]) || data[i] == '_' || data[i] == '-' || data[i] == '.' || data[i] == '\\' || data[i] == '/')
        ++i;
    return i;
}

static efi_status_t add_module(struct loader* loader, const char* name, const uint16_t* path)
{
    if (loader->modules == loader->module_capacity) {
        ...
        status = loader->system->boot->allocate_pool(EFI_LOADER_DATA, new_size * sizeof(struct module), (void**)&new_module);
...
efi_status_t load_config(struct loader* loader, const uint16_t* config_path)
{
    ...
    status = loader->rootdir->open(loader->rootdir, &loader->config, (uint16_t*)config_path, EFI_FILE_MODE_READ, EFI_FILE_READ_ONLY);
    if (status != EFI_SUCCESS) {
    ...
    status = loader->config->get_info(loader->config, &guid, &size, (void*)&file_info);
    if (status != EFI_SUCCESS) {
        err(loader->system, "failed to find size of %w: %llu\r\n", config_path, (unsigned long long)status);
        return status;
    }

    status = loader->system->boot->allocate_pool(EFI_LOADER_DATA, file_info.file_size + 1, (void**)&loader->config_data);
    if (status != EFI_SUCCESS) {
    ...
    return efi_read_fixed(loader->system,
                          loader->config,
                          /* offset */ 0,
...
efi_status_t parse_config(struct loader* loader)
{
    ...
    while (1) {
        ...
        status = loader->system->boot->allocate_pool(EFI_LOADER_DATA, name_size + 1, (void**)&name);
        if (status != EFI_SUCCESS) {
        ...
        status = loader->system->boot->allocate_pool(EFI_LOADER_DATA, 2 * (path_size + 1), (void**)&path);
        if (status != EFI_SUCCESS) {
