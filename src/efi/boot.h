...
typedef enum EFI_TIMER_DELAY { TimerCancel, TimerPeriodic, TimerRelative } EFI_TIMER_DELAY;

typedef enum EFI_ALLOCATE_TYPE { AllocateAnyPages, AllocateMaxAddress, AllocateAddress, MaxAllocateType } EFI_ALLOCATE_TYPE;
...
typedef enum EFI_INTERFACE_TYPE { EFI_NATIVE_INTERFACE } EFI_INTERFACE_TYPE;

typedef enum EFI_LOCATE_SEARCH_TYPE { AllHandles, ByRegisterNotify, ByProtocol } EFI_LOCATE_SEARCH_TYPE;
...
typedef EFI_STATUS(EFIAPI* EFI_GET_MEMORY_MAP)(IN OUT UINTN* MemoryMapSize, OUT EFI_MEMORY_DESCRIPTOR* MemoryMap, OUT UINTN* MapKey, OUT UINTN* DescriptorSize, OUT uint32_t* DescriptorVersion);
...
typedef EFI_STATUS(EFIAPI* EFI_CREATE_EVENT)(IN uint32_t Type, IN EFI_TPL NotifyTpl, IN EFI_EVENT_NOTIFY NotifyFunction, IN VOID* NotifyContext, IN EFI_GUID* EventGroup, OUT EFI_EVENT* Event);
...
typedef EFI_STATUS(EFIAPI* EFI_IMAGE_LOAD)(IN bool BootPolicy, IN EFI_HANDLE ParentImageHandle, IN EFI_DEVICE_PATH_PROTOCOL* DevicePath, IN VOID* SourceBuffer, IN UINTN SourceSize, OUT EFI_HANDLE* ImageHandle);
typedef EFI_STATUS(EFIAPI* EFI_IMAGE_START)(IN EFI_HANDLE ImageHandle, OUT UINTN* ExitDataSize, OUT char16_t** ExitData);
typedef EFI_STATUS(EFIAPI* EFI_EXIT)(IN EFI_HANDLE ImageHandle, IN EFI_STATUS ExitStatus, IN UINTN ExitDataSize, IN char16_t* ExitData);
...
typedef EFI_STATUS(EFIAPI* EFI_SET_WATCHDOG_TIMER)(IN UINTN Timeout, IN uint64_t WatchdogCode, IN UINTN DataSize, IN char16_t* WatchdogData);
typedef EFI_STATUS(EFIAPI* EFI_CONNECT_CONTROLLER)(IN EFI_HANDLE ControllerHandle, IN EFI_HANDLE* DriverImageHandle, IN EFI_DEVICE_PATH_PROTOCOL* RemainingDevicePath, IN bool Recursive);
...
typedef EFI_STATUS(EFIAPI* EFI_CALCULATE_CRC32)(IN VOID* Data, IN UINTN DataSize, OUT uint32_t* Crc32);
typedef EFI_STATUS(EFIAPI* EFI_COPY_MEM)(IN VOID* Destination, IN VOID* Source, IN UINTN Length);
typedef EFI_STATUS(EFIAPI* EFI_SET_MEM)(IN VOID* Buffer, IN UINTN Size, IN uint8_t Value);
typedef EFI_STATUS(EFIAPI* EFI_CREATE_EVENT_EX)(IN uint32_t Type, IN EFI_TPL NotifyTpl, IN EFI_EVENT_NOTIFY NotifyFunction, IN VOID* NotifyContext, IN EFI_GUID* EventGroup, OUT EFI_EVENT* Event);
